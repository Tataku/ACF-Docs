...
**"Earned Conviction Ladder" does not exist as a named artifact in any repo** (ACFDashboard, chaz-automation,
ACF-Docs; searched under every plausible phrasing including the six stage names). Ruling for Chart 5.2:

- The six evidence stages (Thesis exposure → Commercial validation → Initial execution → Scale execution →
  Economic proof → Exceptional platform quality) ship as **owner-supplied conceptual narrative**
  (`dataMode: conceptual`).
- The sizing overlay uses **only canonical numbers**: the four CIS bands (0% · 2–4% · 4–8% · 8–15%) plus the
  15% default single-position cap and 18% documented-override absolute maximum.
- The chart asserts **no invented stage→percentage mapping**. Evidence advances the score; the score advances
  the band; governance caps the ceiling. That is the exact canonical mechanism.
...
| Hype | 1–5% per position (5% hard max) · 10% aggregate cap · stops 15–25% · time limit 3–6 months | CIS v2.1 §10.2 · Part 5 |
...
| 5.2 | `p5-earned-size` | Torque Position Sizing (first sizing section) | four-band table | Table shows score→size but not *why* size must be earned over time | Staircase makes "evidence advances → band advances → cap holds" one glance | Four bands 0/2–4/4–8/8–15 · caps 15/18; stages are conceptual (no repo doctrine — §1.1 ruling) | `rangeSteps` (NEW) | ◌ | Six stage labels at 390px — abbreviate + full label in tap rail |
| 5.3 | `p5-posture-sizing` | After the sizing tables (Torque+Ballast+Hype read together) | three tables | Same CIS ≠ same size across postures is stated but never *seen*; guardrails scattered across prose | Aligned vertical ranges expose the posture asymmetry instantly; guardrail rules overlay | Torque/Ballast/Hype band sets · 10% Ballast single max · 15/18 · 35/50 · 10% Hype agg · 40% Ballast agg | `rangeSteps` (NEW, columns mode) | ◇ | Three columns fit 390px if compact; guardrail labels right-aligned |
