...
for (const s of FRAMEWORK_CHART_SPECS) {
  ...
  if (s.layout === 'quadrant') {
    ...
    // reflexivity as two mirrored loops that share three central mechanism cards
    ...
    loops.forEach((lp, li) => ok(Array.isArray(lp.states) && lp.states.length === 4, `${w} feedbackLoop loop ${li} needs exactly 4 states`));
    const seq = (loops[0]?.states || []).map((n) => n.id);
    ok(new Set(seq).size === seq.length, `${w} feedbackLoop has duplicate state ids`);
    // both loops must share the same ordered state ids (a hover lights the state in both loops)
    loops.forEach((lp, li) => ok(JSON.stringify((lp.states || []).map((n) => n.id)) === JSON.stringify(seq), `${w} feedbackLoop loop ${li} state ids must match loop 0`));
    // three shared mechanism cards, each pinned to a row
    ok(Array.isArray(fl.mechanisms) && fl.mechanisms.length === 3, `${w} feedbackLoop needs exactly 3 mechanisms`);
    (fl.mechanisms || []).forEach((m) => ok(m.id && m.title && ['top', 'mid', 'bot'].includes(m.row), `${w} feedbackLoop mechanism needs id, title, and row in {top,mid,bot}`));
    ok(seq.includes(s.primaryKey), `${w} primaryKey ${s.primaryKey} is not a feedbackLoop state`);
    ok(s.hoverTargets.some((t) => t.id === s.primaryKey), `${w} primaryKey ${s.primaryKey} not a hover target`);
    ...
    const rlx = s.signature || {};
    ok(rlx.shared && typeof rlx.shared === 'string', `${w} signature needs a shared (price) stage`);
    ok(Array.isArray(rlx.reinforce) && rlx.reinforce.length >= 4, `${w} signature reinforce lobe needs >= 4 stages`);
    ok(Array.isArray(rlx.reverse) && rlx.reverse.length >= 4, `${w} signature reverse lobe needs >= 4 stages`);
    ok((rlx.reinforce || []).some((n) => n.stage === 'buildout'), `${w} signature reinforce lobe missing explicit buildout stage`);
    ok((rlx.reverse || []).some((n) => n.stage === 'buildout'), `${w} signature reverse lobe missing explicit buildout stage`);
    // both lobes must share the same ordered satellite stage ids
    const rSeq = (rlx.reinforce || []).map((n) => n.stage);
    ok(JSON.stringify(rSeq) === JSON.stringify((rlx.reverse || []).map((n) => n.stage)), `${w} signature reverse lobe stage ids must match reinforce`);
    const nodeIds = [...new Set([rlx.shared, ...rSeq])];
    ok(nodeIds.length >= 3, `${w} signature needs >= 3 distinct stages`);
    ok(nodeIds.includes(s.primaryKey), `${w} primaryKey ${s.primaryKey} is not a signature stage`);
    ok(s.primaryKey === rlx.shared, `${w} signature primaryKey must be the shared (crossing) stage`);
    ok(!rlx.annotations || (Array.isArray(rlx.annotations) && rlx.annotations.length <= 3), `${w} signature allows at most 3 annotations`);
    (rlx.annotations || []).forEach((an) => ok(nodeIds.includes(an.from) && nodeIds.includes(an.to) && an.text, `${w} signature annotation must tie two real stages with text`));
    // every stage id needs a hover target
    nodeIds.forEach((id) => ok(s.hoverTargets.some((t) => t.id === id), `${w} signature stage ${id} has no hover target`));
  } else {
