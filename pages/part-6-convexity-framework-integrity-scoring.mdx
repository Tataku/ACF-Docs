...
- **Part 6:** Convexity & Framework Integrity Scoring *(this document)*  
...
<Callout type="info" title="Product scoring extensions (not core doctrine)">

The ACF Dashboard product also surfaces additional, product-level scoring layers—the **Framework Fit engine** and the **Doctrine Audit**—that build on CIS/FIS to answer product-facing questions (for example, "which doctrine does this portfolio express?"). These are **product scoring extensions, not part of the core CIS/FIS doctrine defined above.** CIS (position quality) and FIS (construction integrity) remain the canonical two-score kernel; the product layers are evaluated and versioned separately.
