...
export const FRAMEWORK_CHART_SPECS = [
...

  {
    ...
    claimStack: {
      ...
      visualProof: 'A wide figure-eight racetrack around a shared PRICE hub: the top half is the reinforcing loop read left to right — price rises, capital flows in, buildout expands, fundamentals improve, validation confirms — sweeping back into price; the bottom half is the reversing loop, an equally-real mirror that runs the same stages the other way and breaks.',
      interactionRole: 'Hover any stage to light its role in both the reinforcing and the reversing loop',
      readerAction: 'Read the top loop left to right, then the bottom — both feed back through price at the centre',
    ...
    chartType: 'A horizontal figure-eight racetrack of ten stage cards around a shared PRICE hub; the top loop (reinforcing) and bottom loop (reversing) each run price → capital → buildout → fundamentals → validation → back into price.',
    visualDataMode: 'conceptual', disclosure: DISCLOSURE.conceptual, footerCta: 'View framework basis',
    ...
    layout: 'feedbackLoop',
    ariaSummary: 'Reflexivity drawn as a wide horizontal figure-eight around a shared price hub at the centre. The top half is the reinforcing loop, read left to right: price rises, capital flows in, buildout expands, fundamentals improve, and validation confirms — then the loop sweeps back into price, its brush stroke in the accent colour. The bottom half is the reversing loop, an equally weighted mirror read left to right: price falls, capital tightens, buildout slows, fundamentals weaken, and validation breaks — then it too sweeps back into price, in the stress colour. Every stage has a clear arrowhead showing the direction of flow, and sparse notes mark the collision points where one mechanism becomes the next: financing changes capacity, execution becomes evidence, and evidence feeds back into price. Both loops meet at the same central price signal, and the core insight sits below: price can write the fundamentals it claims to read.',
    feedbackLoop: {
      eyebrow: 'reflexivity',
      punchline: 'Price can write the fundamentals it claims to read.',
      shared: { id: 'price', label: 'Price' },
      loops: [
        {
          id: 'reinforce', label: 'REINFORCING', sub: 'momentum compounds', tone: 'accent',
          nodes: [
            { id: 'price', label: 'Price rises' },
            { id: 'capital', label: 'Capital flows in' },
            { id: 'buildout', label: 'Buildout expands' },
            { id: 'fundamentals', label: 'Fundamentals improve' },
            { id: 'validation', label: 'Validation confirms' },
          ],
          micro: [
            { anchor: 'capital', text: 'financing changes capacity', dy: -3 },
            { anchor: 'validation', text: 'execution becomes evidence', dy: -3 },
          ],
        },
        {
          id: 'reverse', label: 'REVERSING', sub: 'momentum contracts', tone: 'stress',
          nodes: [
            { id: 'price', label: 'Price falls' },
            { id: 'capital', label: 'Capital tightens' },
            { id: 'buildout', label: 'Buildout slows' },
            { id: 'fundamentals', label: 'Fundamentals weaken' },
            { id: 'validation', label: 'Validation breaks' },
          ],
          micro: [
            { anchor: 'validation', text: 'evidence feeds back into price', dy: 4 },
          ],
        },
      ],
    },
    ...
    hoverTargets: [
      ...
      { id: 'capital', kind: 'stage', label: 'Capital', name: 'Capital', why: 'Responds to the price signal — this is where financing conditions change. A rising price draws it in with cheaper financing, talent and attention; a falling price tightens and withdraws them. Capital is the transmission belt from perception to the real buildout.', claim: 'Capital follows the signal and funds — or starves — the buildout.', concept: 'Liquidity cycle', link: '/part-2-lineage-macro-thesis' },
      { id: 'buildout', kind: 'stage', label: 'Buildout', name: 'Buildout · execution', why: 'Where capital becomes real activity — hiring, capacity, product. This is the bridge from capital to fundamentals: funded, it expands and execution scales; starved, it slows and the improvement never lands. The step abstract loop diagrams usually skip.', claim: 'Capital only bends fundamentals by funding real execution.', concept: 'Execution', link: '/part-2-lineage-macro-thesis' },
      { id: 'fundamentals', kind: 'stage', label: 'Fundamentals', name: 'Fundamentals', why: 'Reflect the real-world execution the buildout produced — this is where execution becomes reality. They improve when it expands and weaken when it slows, bending toward the price with a lag, not instantly, in whichever loop is turning.', claim: 'The fundamentals bend toward the price.', concept: 'Reflexivity', link: '/part-2-lineage-macro-thesis' },
    ...
    implementationNotes: 'feedbackLoop layout redesigned as a HORIZONTAL figure-eight racetrack (owner brief 2026-07, refined direction), built from the Soros reflexivity text. Ten stage cards ring a shared PRICE hub at the centre: the top half is the reinforcing loop read left→right (price rises · capital flows in · buildout expands · fundamentals improve · validation confirms), the bottom half the reversing loop read left→right (price falls · capital tightens · buildout slows · fundamentals weaken · validation breaks). Each half sweeps back into the hub. Loops are drawn as confident, BALANCED brush strokes (Brush.brushLine) with clear brush arrowheads (Brush.brushArrow) at every stage — neither loop dominates. Five shared stage ids (price · capital · buildout · fundamentals · validation); the price-rises / price-falls cards and the central hub all resolve to the price stage, so hovering price lights all three. Sparse marginal micro-explainers name the collision points (financing changes capacity · execution becomes evidence · evidence feeds back into price). The Price hub border is an accent→stress vertical gradient — both loops meet at the same signal. The punchline — "Price can write the fundamentals it claims to read" — sits below. Guardrails honored in copy: fundamentals bend with a lag (not instantly), validation does not always follow, the mechanism is a tendency not a formula, and the reverse loop is drawn as equally real. Mobile compacts card labels to the stage noun (Price ↑ / ↓). Supersedes the vertical figure-eight, two-track causal-story, spiral, two-lane, single-spine, and two-node attempts.',
  },
