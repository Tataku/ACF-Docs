...
export const FRAMEWORK_CHART_SPECS = [
...

  {
    ...
    claimStack: {
      ...
      visualProof: 'Two circular flows drawn as a figure-eight that crosses at PRICE: a reinforcing loop on top — price draws capital in, funds the buildout, improves fundamentals, and the validation feeds back into price — and a reversing loop below that runs the same circle in reverse and breaks.',
      interactionRole: 'Hover any stage to light its role in both the virtuous and the vicious loop',
      readerAction: 'Trace either loop around and back through price — the shared hinge where they cross',
    ...
    chartType: 'A figure-eight of two circular flows (reinforcing / reversing) tangent at a shared PRICE hinge; each loop runs price → capital → fundamentals → validation → back into price.',
    visualDataMode: 'conceptual', disclosure: DISCLOSURE.conceptual, footerCta: 'View framework basis',
    ...
    layout: 'feedbackLoop',
    ariaSummary: 'Reflexivity drawn as a figure-eight of two circular flows that cross at a shared price hinge in the middle. The top loop is reinforcing, a virtuous cycle: a rising price draws capital in, the capital funds the buildout, the buildout improves fundamentals, and the improved fundamentals appear to validate the price — which feeds back into price. Its ring is a solid accent stroke whose arrows thicken to show momentum compounding. The bottom loop is reversing, a vicious cycle: a falling price drives capital out, the buildout is starved, fundamentals weaken, and the validation breaks — its ring is a thinner dashed stress stroke with a jagged snap where the loop fails. Both loops pivot on the same price node where they cross, and the core insight sits below: price can write the fundamentals it claims to read.',
    feedbackLoop: {
      eyebrow: 'reflexivity',
      ...
      loops: [
        {
          id: 'reinforce', label: 'REINFORCING', sub: 'price rises · virtuous cycle', tone: 'accent',
          edgeLabel: 'buildout funded',
          nodes: [
            { id: 'capital', label: 'Capital in' },
            { id: 'fundamentals', label: 'Fundamentals improve' },
            { id: 'validation', label: 'Validation confirms' },
          ],
        },
        {
          id: 'reverse', label: 'REVERSING', sub: 'price falls · vicious cycle', tone: 'stress',
          edgeLabel: 'buildout starved',
          nodes: [
            { id: 'capital', label: 'Capital out' },
    ...
    primaryKey: 'price',
    hoverTargets: [
      { id: 'price', kind: 'stage', label: 'Price', name: 'Price — the hinge', why: 'Where the two loops cross. A rising price pulls capital in and the virtuous loop turns; a falling price drives it out and the vicious loop turns. Either way price is a participant in the system, not just a readout of it — every loop returns here.', claim: 'Price moves the market it claims to measure.', concept: 'Reflexivity', link: '/part-2-lineage-macro-thesis' },
      { id: 'capital', kind: 'stage', label: 'Capital', name: 'Capital', why: 'Follows the price signal. When price rises it arrives with cheaper financing, talent and attention; when price falls it withdraws them. Capital is the transmission belt between perception and the real buildout — where "buildout funded" becomes "buildout starved".', claim: 'Capital follows the signal and funds — or starves — the story.', concept: 'Liquidity cycle', link: '/part-2-lineage-macro-thesis' },
      { id: 'fundamentals', kind: 'stage', label: 'Fundamentals', name: 'Fundamentals', why: 'Genuinely improve when the buildout is funded — so the reading becomes partly self-fulfilling — and weaken when it is starved. They bend toward the price with a lag, not instantly, in whichever loop is turning.', claim: 'The fundamentals bend toward the price.', concept: 'Reflexivity', link: '/part-2-lineage-macro-thesis' },
      { id: 'validation', kind: 'stage', label: 'Validation', name: 'Validation', why: 'Improved fundamentals appear to ratify the original move, and that apparent confirmation feeds back into price — closing the virtuous loop. But validation does not always follow: when fundamentals weaken the confirmation breaks, and the vicious loop closes instead.', claim: 'Confirmation feeds back into price — until it breaks.', concept: 'Macro thesis', link: '/part-2-lineage-macro-thesis' },
    ],
    mobileTapTargets: ['price', 'capital', 'fundamentals', 'validation'],
    implementationNotes: 'feedbackLoop layout redesigned as a FIGURE-EIGHT of two circular flows (owner brief 2026-07), built from the Soros reflexivity text. Two ellipse loops are tangent at a single shared PRICE node (the waist / hinge, drawn once): top loop reinforcing (accent, solid ring, brush chevrons thicken with momentum), bottom loop reversing (stress, dashed ring, chevrons thin + a jagged snap on the closing arc). Each loop runs price → capital → fundamentals → validation → back to price; "buildout" is the labelled mechanism on the capital→fundamentals arc (funded vs starved). Four shared stage ids (price · capital · fundamentals · validation); hovering a stage lights it in both loops. The punchline — "Price can write the fundamentals it claims to read" — sits below the crossing. Guardrails honored in copy: fundamentals bend with a lag (not instantly), validation does not always follow, the mechanism is a tendency not a formula, and the reverse loop is drawn as equally real. Arrows use the design-system brush (Brush.brushArrow). Supersedes the two-track causal-story, spiral, two-lane, single-spine, and two-node attempts.',
  },
