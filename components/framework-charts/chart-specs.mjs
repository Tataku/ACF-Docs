...
export const FRAMEWORK_CHART_SPECS = [
...

  {
    ...
    claimStack: {
      ...
      visualProof: 'Two nodes — price and fundamentals — joined by a quiet dashed "reads" arc (the claim that price reflects value) and a bold accent "writes" arc (the reality that price moves capital and rewrites value)',
      interactionRole: 'Hover the reads and writes arcs to weigh the claim against the reality',
      readerAction: 'Read the writes arc — the one that should not exist',
    ...
    chartType: 'Self-reference diagram: price reads the fundamentals (the claim) and writes them (the reality) — two arcs between two nodes.',
    visualDataMode: 'conceptual', disclosure: DISCLOSURE.conceptual, footerCta: 'View framework basis',
    ...
    layout: 'feedbackLoop',
    ariaSummary: 'Reflexivity shown as a self-reference between two nodes, Price and Fundamentals. A quiet dashed arc above reads from fundamentals to price — the conventional claim that price merely reflects the fundamentals. A bold accent arc below writes from price back to fundamentals — the reflexive reality, that price moves capital and so changes the fundamentals it claims only to read.',
    feedbackLoop: {
      eyebrow: 'reflexivity',
      readLabel: 'reads',
      writeLabel: 'writes',
      caption: 'price can write the fundamentals it claims to read',
      nodes: [
    ...
    primaryKey: 'price',
    hoverTargets: [
      { id: 'price', kind: 'node', label: 'Price', name: 'Price', why: 'Claims only to read the fundamentals — to reflect their value. But it also moves capital, and that capital rewrites the very fundamentals it was supposedly just measuring.', claim: 'Price is the author, not only the mirror.', concept: 'Reflexivity', link: '/part-2-lineage-macro-thesis' },
      { id: 'fundamentals', kind: 'node', label: 'Fundamentals', name: 'Fundamentals', why: 'The story price claims to read. Capital drawn by a rising price genuinely improves them; capital fleeing a falling one hollows them out — so the reading becomes self-fulfilling.', claim: 'The fundamentals bend to the price that reads them.', concept: 'Reflexivity', link: '/part-2-lineage-macro-thesis' },
      { id: 'reads', kind: 'link', label: 'Reads', name: 'Reads — the claim', why: 'The conventional view: price passively reflects the fundamentals, a mirror held up to value.', claim: 'What price claims to do.', concept: 'Efficient markets', link: '/part-2-lineage-macro-thesis' },
      { id: 'writes', kind: 'link', label: 'Writes', name: 'Writes — the reality', why: 'The reflexive truth: by moving capital toward or away from an asset, price changes the fundamentals it claims to measure. The mirror edits the room.', claim: 'What price actually does.', concept: 'Reflexivity', link: '/part-2-lineage-macro-thesis' },
    ],
    mobileTapTargets: ['price', 'writes', 'fundamentals', 'reads'],
    implementationNotes: 'feedbackLoop layout, redesigned to the minimal "price writes the story" self-reference: two nodes (price · fundamentals) with a muted dashed reads arc (the conventional claim) and a bold accent writes arc (the reflexive reality — the whole insight), under the caption "price can write the fundamentals it claims to read." The mechanism (capital) lives in the hover + explainer, keeping the face to one idea. Supersedes the two-lane, single-spine, and systemLoop-ring attempts.',
  },
