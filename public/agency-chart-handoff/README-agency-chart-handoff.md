...
- **Control-first picture — match the axis to the claim.** When the concept is *control*, do **not** lead with a portfolio-value-over-time line — the eye scores it by terminal value and the framework looks like a mediocre middle line. *Exposure Is Not Control* is a **Control vs Participation map**: x = participation (exposure), y = control (followability), with a soft **governable zone** on the tradeoff frontier. The framework sits inside it; max exposure is far-right and low/spread (high participation, fragile); stress-tested is upper-left (high control, cash drag). The picture itself says *exposure and control are different dimensions.*
...
- **Framework diagrams** (bespoke, brush-influenced, not flowcharts):
  ...
  - `scenario` — an interactive **Control vs Participation map** (NOT a return path): x = participation (exposure), y = control (followability), with a soft governable **zone** on the tradeoff frontier. Each strategy is a vertical cluster — participation is per-strategy, the selected shock sets control; the other shocks are faint dots, so a short cluster reads as repeatable. The framework sits inside the zone; max exposure far-right + low/spread (fragile); stress-tested upper-left (defensive, cash drag). Read-outs lead with control / participation / decision strain / forced-error risk / livability; terminal + drawdown are subordinate outcomes. Strategy/shock selectors drive it; hover/click a mark to inspect/select (e.g. *Exposure Is Not Control*). The picture is the claim: exposure and control are different dimensions.
