...
- **Tap opens the pinned tooltip AT the element.** On coarse a tap surfaces the same pinned `TargetTooltip` (anchored connector, brush frame, ✕ dismiss, READ link) at the tapped element, alongside the `MobileInsight` rail — touch readers get the tooltip itself, not only the rail. Background tap, ✕, or FULL VIEW dismisses.
