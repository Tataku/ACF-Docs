...
- **The container is NEVER resized for mobile — type is a register, not a scale.** The viewBox keeps the layout's authored proportions on every viewport (the sizing rules above: orchestrator heights + `tall` vertical room are the only size accommodations). Changing the design-space aspect on coarse DISTORTS the authored geometry — a data curve drawn into a squarer space is a different visual claim. Legibility comes from a larger authored TYPE REGISTER on `coarse` (~1.8× in-plot fonts — user-px type changes nothing about the geometry) plus proportionally bolder imagery (markers, dots, ports, path weights). The one exception is an owner-approved re-authored diagram composition (feedbackLoop's 500×440 mobile form) — an explicit design decision, never a default.
- **NO floating tooltips on mobile — LOCKED (owner decision, re-affirmed 2026-07-13).** Floating tooltip cards are not legible at phone scale; that is exactly WHY the `MobileInsight` rail exists — the rail below the plot IS the mobile tooltip surface. On coarse, a tap selects the element into the rail (and the stepper walks the rest); `TargetTooltip` is desktop-only, gated behind `!coarse` at every renderer site. This was tried and intentionally removed twice — do not re-introduce a coarse/tap tooltip path.
